...
test_samples = Sample.read("data/Practice Data - Combined.tsv", "\t")
keys = make_keys(3, 2)
cells = new_cells.make_cells(test_samples, keys)
cells = new_cells.calc_cells(test_samples, ["SNP1", "SNP4"], cells)

for key in keys:
    print key
    print "Case: ", cells[key].case
    print "Control: ", cells[key].control
